...
class FunASRNano(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        print("\n\n🔥🔥🔥 [VERIFIED] 加载的是本地修改版 FunASR Nano 模型 (Batch Optimized) 🔥🔥🔥\n\n")
        # audio encoder
    ...
    def inference_prepare(
        ...
        **kwargs,
        ...
        batch_outputs = []
        for item in data_in:
            contents = self.data_template(item)
            output = self.data_load_speech(
                contents, tokenizer, frontend, meta_data=meta_data, **kwargs
            )
            batch_outputs.append(output)

        # 2. Collate (Merge) results into a single batch dict
        if len(batch_outputs) == 1:
            batch_data = batch_outputs[0]
        else:
            batch_data = {}
            # Flatten speech tensors from all samples
            all_speech = []
            all_speech_lens = []
            
            # Lists for padding other fields
            batch_fields = {
                "input_ids": [], "labels_ids": [], "source_ids": [], "target_ids": [],
                "attention_mask": [], "fbank_mask": [], "fake_token_len": [], "fbank_beg": []
            }

            for out in batch_outputs:
                # Handle speech flattening
                s = out.get("speech")
                sl = out.get("speech_lengths")
                if s is not None and len(s) > 0:
                    # s is [N_audio, T, D]
                    for i in range(s.shape[0]):
                        all_speech.append(s[i])
                        all_speech_lens.append(sl[i])
                
                # Handle other fields (squeeze batch dim 0 to get [L] or [T])
                for k in batch_fields.keys():
                    if k in out:
                        batch_fields[k].append(out[k].squeeze(0))

            # Pad speech
            if all_speech:
                batch_data["speech"] = torch.nn.utils.rnn.pad_sequence(all_speech, batch_first=True, padding_value=0.0)
                batch_data["speech_lengths"] = torch.cat(all_speech_lens).unsqueeze(1) # [Total_Speech, 1]
            else:
                batch_data["speech"] = []
                batch_data["speech_lengths"] = []

            # Pad other fields
            for k, tensor_list in batch_fields.items():
                if not tensor_list:
                    batch_data[k] = torch.tensor([])
                    continue
                
                pv = -100 if k == "labels_ids" else 0
                batch_data[k] = torch.nn.utils.rnn.pad_sequence(tensor_list, batch_first=True, padding_value=pv)

        # 3. Move to device
        batch = to_device(batch_data, kwargs["device"])
        ...
        speech_idx = 0
        for batch_idx in range(batch_size):
            if fbank_beg.shape[0] <= batch_idx: break
    ...
    def inference(
        ...
        **kwargs,
        ...
        # 🔥🔥🔥 核心修改区：动态 prompt 注入 🔥🔥🔥
        # 获取传入的语言参数 (默认为 auto)
        target_lang = kwargs.get("language", "auto")
        
        # 定义语言与提示词的映射关系
        # 这里的提示词必须是 Qwen 模型能听懂的“母语”
        prompt_map = {
            "zh": "以下是中文语音的转写：",                    # 用中文开头
            "en": "The following is English speech transcription: ",  # 用英文开头
            "ja": "以下は日本語音声の書き起こしです：",         # 用日文开头
            "ko": "다음은 한국어 음성 전사입니다: ",           # 用韩文开头
            "yue": "以下係粵語語音嘅轉寫：",                   # 用粤语开头
            "auto": ""                                          # 不加前缀
        }
        
        # 选取前缀，如果没有命中则用默认值
        prompt_prefix = prompt_map.get(target_lang, "")
        
        logging.info(f"[Nano] Language='{target_lang}', Using Prompt='{prompt_prefix}'")
                ...
                new_data_in.append(
                    [
                        {"role": "system", "content": "You are a helpful assistant."},
                        {
                            "role": "user",
                            # 使用动态前缀替换死板的 "语音转写："
                            "content": f"{prompt_prefix}<|startofspeech|>!{data}<|endofspeech|>",
                        },
                ...
                new_data_in.append(
                    [
                        {"role": "system", "content": "You are a helpful assistant."},
                        {
                            "role": "user",
                            # 使用动态前缀
                            "content": f"{prompt_prefix}<|startofspeech|>!!<|endofspeech|>",
                            "audio": data,
                        },
                        {"role": "assistant", "content": "null"},
                    ]
