...
### ⚡ Qwen3-ASR 推理吞吐表现 (v0.8.1 调优成果)

我们在 NVIDIA RTX 4080 Laptop (12GB 显存) 上，针对 10 路短音频并发和单路 34.3 分钟长音频进行 Qwen3-ASR 推理基准测试，数据如下：

* **极速离线转写**：中长音频转写实时率 (RTF) 平均为 **0.029**，即达到 **34倍速** 转写（34.3 分钟音频仅需 58.9 秒完成推理）。
* **端到端 VAD 对齐转写**：在完整对齐流水线（VAD 分割 ➔ 宿主机-Docker 共享卷直读 ➔ vLLM 并发推理 ➔ 强制对齐时间轴拼装）下，34.3分钟音频端到端整体耗时 **85.373秒**，RTF 仅为 **0.041**（即 **24.2倍速**）。
* **Docker 共享存储卷直读（Direct Path）优化**：相比传统的 Base64 HTTP 传输，宿主机 SDK 直接通过 Docker 共享临时卷 `/app/server/shared_tmp` 传递容器内绝对路径，在批量转写中成功**节省了高达 51.5% 的网络和编解码开销时间**，有效杜绝大网络载荷引发的内存暴涨与超时。
