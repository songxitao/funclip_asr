...
* **P3.1/P3.3 离线归口与大文件清理**：物理删除 `funclip/` 下 5 个 0 引用大文件副本。将 [app_control.py](file:///E:/project/funclip-pro/app_control.py) 里的离线转写完全重构为进程内惰性加载调用 `OfflinePipeline`，本地直接写出 `.txt` 与 `.srt`，完全替代原有的 `subprocess` CMD 弹窗调用，并彻底物理清理孤立的 `asr1.py` 和 `launch.py` 链。
...
- **Gradio 离线限制拦截**：`app_control.py` 中虽然保留了旧的 Whisper/Qwen3 等引擎界面选项，但在进程内直接调用时已增加友好拦截保护。如果后续要支持这些引擎，必须先对它们进行 Pipeline 包化。
