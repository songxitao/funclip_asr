"""pyannote segmentation-3.0 推理引擎（core 下沉版）。
...
本模块由 P0 的 segmentation_engine.py 迁移而来，推理逻辑保持字节级等价。
DLL 补丁由 config.loader 在导入时执行；import funclip_pro.config.loader 即触发。
"""

# 绝对导入：使用项目包路径，避免跨级相对 import。
...
class SegmentationEngine:
    ...
    def _process_chunk_seamless(
        ...
        for f in range(1, num_frames):
            if frame_type[f] != cur_type:
                ...
                end_sec = f * frame_sec
                end_sec = min(end_sec, actual_duration)
                label = self._type_label(cur_type)
                if label == "single":
                    # 应用 min_seg_sec 过滤
                    if end_sec - start_sec >= min_seg_sec:
    ...
    def _type_label(self, ft: int) -> str:
        """帧类型 → 段类型字符串（私有辅助）。

        -1 → "silence"；3 → "overlap"；0/1/2 → "single"。
        """
        if ft == -1:
            return "silence"
        elif ft == 3:
