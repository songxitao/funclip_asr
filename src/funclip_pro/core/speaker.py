"""
core/speaker.py — 基于 Cam++ (speech_campplus_sv) 的说话人向量提取 + 离线聚类
...
本文件由根 speaker_engine.py 下沉而来，算法逻辑字节级等价，未做行为改动。
"""
...
logger = __import__("logging").getLogger("ASRService.Speaker")

# 说话人模型 DLL 补丁保活：必须在首次加载 torch / onnxruntime / campplus 之前点亮
# DLL 搜索目录（等价 asr_onnx_service.py 顶部调用）。非 Windows 平台安全空转。
apply_dll_patch()
...
class CampPlusSpeaker:
    ...
    def cluster_with_segmentation(self, audio_16k: np.ndarray, segment_engine: "SegmentationEngine",
    ...
    def cluster_with_seamless_segmentation(
        ...
        segment_engine: "SegmentationEngine",
        sr: int = 16000,
        n_speakers: Optional[int] = None,
